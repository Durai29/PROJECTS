...
// Choose color based on frame count (using rotation variable A)
int frame_color = 31 + ((int)(A * 5) % 6);  // Cycles through 6 colors

for(int k=0; k<WIDTH*HEIGHT; k++) {
    if(k % WIDTH) {
        if(b[k] != ' ') {  // Only color the donut pixels
            printf("\x1b[%dm%c", frame_color, b[k]);
        } else {
            putchar(' ');  // Leave background spaces uncolored
        }
    } else {
        printf("\n");
    }
}
printf("\x1b[0m");  // Reset color at end of framei
                A += 0.08;
                B += 0.04;
                Sleep(5);
        }
